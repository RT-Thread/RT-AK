...
static mp_obj_t py_sensor_snapshot(uint n_args, const mp_obj_t *args, mp_map_t *kw_args)
{
    ...
    image_t *_cobj = (image_t *) py_image_cobj(image);
    mp_obj_t image_ai = py_image(_cobj->w, _cobj->h, IMAGE_BPP_RGB888_CHW, _cobj->ai_pixels);

    mp_obj_list_t *img_list = NULL;
    img_list = m_new(mp_obj_list_t, 1);
    mp_obj_list_init(img_list, 0);
    mp_obj_list_append(img_list, image );
    mp_obj_list_append(img_list, image_ai );
    return img_list;
}
...
static mp_obj_t py_sensor_switch(mp_obj_t pos_arg) 
{
    if (mp_obj_get_type(pos_arg) != &mp_type_int)
        mp_raise_ValueError("Invalid Value");
    
    int select = mp_obj_get_int(pos_arg);
    if (select == 1)
        sensor_switch(1);
    else if (select == 0)
        sensor_switch(0);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(py_sensor_switch_obj,              py_sensor_switch);
...
STATIC const mp_map_elem_t globals_dict_table[] = {
    ...
    { MP_OBJ_NEW_QSTR(MP_QSTR_sw),                  (mp_obj_t)&py_sensor_switch_obj},
};
