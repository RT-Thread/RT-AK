...
static void py_image_print(const mp_print_t *print, mp_obj_t self_in, mp_print_kind_t kind)
{
    ...
    switch(self->_cobj.bpp) {
        case IMAGE_BPP_RGB888_CHW: {
            mp_printf(print, "{\"w\":%d, \"h\":%d, \"type\"=\"rgb888_chw\", \"size\":%d}",
                      self->_cobj.w, self->_cobj.h,
                      (self->_cobj.w * self->_cobj.h) * 3);
            break;
        }
        case IMAGE_BPP_BINARY: {
...
static mp_obj_t py_image_format(mp_obj_t img_obj)
{
    switch (((image_t *) py_image_cobj(img_obj))->bpp) {
        ...
        case IMAGE_BPP_RGB888_CHW: return mp_obj_new_int(PIXFORMAT_BAYER);
...
static mp_obj_t py_image_to_rgb565(uint n_args, const mp_obj_t *args, mp_map_t *kw_args)
{
    ...
    switch(arg_img->bpp) {
        ...
        case IMAGE_BPP_RGB888_CHW: {
            PY_ASSERT_TRUE_MSG( copy, "For rgb888_chw, arg `copy` must be `True`!!!");
            image_t *out_ptr = &out;
            for(uint32_t ih = 0; ih < out_ptr->h; ih++){
                for(uint32_t iw = 0; iw < out_ptr->w; iw++){
                    uint8_t r = IMAGE_GET_RGB888_CHW_PIXEL(arg_img, iw, ih, 0);
                    uint8_t g = IMAGE_GET_RGB888_CHW_PIXEL(arg_img, iw, ih, 1);
                    uint8_t b = IMAGE_GET_RGB888_CHW_PIXEL(arg_img, iw, ih, 2);
                    IMAGE_PUT_RGB565_PIXEL(out_ptr, iw, ih, COLOR_R8_G8_B8_TO_RGB565(r, g, b));
                }
            }
            break;
        }
...
#define __log(_val) 
// mp_printf(&mp_plat_print,"%s,%d," #_val "=%d\n",__FILE__,__LINE__,_val);
...
static mp_obj_t py_image_copy_int(uint n_args, const mp_obj_t *args, mp_map_t *kw_args, bool mode)
{
    ...
    if (copy_to_fb_obj) {
        if (mp_obj_is_integer(copy_to_fb_obj)) {
            ...
            } else if (mp_obj_get_int(copy_to_fb_obj)) {__log(0);
                arg_other = NULL;
            }
        } else {__log(0);
    ...
    switch(arg_img->bpp) {
        ...
        case IMAGE_BPP_RGB888_CHW: {
            image_t image2;
            image_t arg_img2;
            memcpy(&arg_img2, arg_img, sizeof(image_t));
            image2=image;
            __log((image_size(arg_img)));
            __log((image_size(&image2)));
            for(int z=0; z<3; z++){
                image2.data += (image.h*image.w*z);
                arg_img2.data += (arg_img->h*arg_img->w*z);
                for (int y = 0, yy = image2.h; y < yy; y++) {
                uint8_t *row_ptr = IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR(&arg_img2, fast_floorf(y * over_yscale) + roi.y);
                uint8_t *row_ptr_2 = IMAGE_COMPUTE_GRAYSCALE_PIXEL_ROW_PTR(&image2, y);
                    for (int x = 0, xx = image2.w; x < xx; x++) {
                        IMAGE_PUT_GRAYSCALE_PIXEL_FAST(row_ptr_2, x,
                            IMAGE_GET_GRAYSCALE_PIXEL_FAST(row_ptr, fast_floorf(x * over_xscale) + roi.x));
                    }
                }
    ...
    py_helper_update_framebuffer(&image);__log(0);
