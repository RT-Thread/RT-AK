...
static int gc0328_read_reg(sensor_t *sensor, uint16_t reg_addr)
{
    uint8_t reg_data;
    if (cambus_readb3(sensor->slv_addr, reg_addr, &reg_data) != 0) {
        return -1;
    }
    return reg_data;
}

static int gc0328_write_reg(sensor_t *sensor, uint16_t reg_addr, uint16_t reg_data)
{
    return cambus_writeb3(sensor->slv_addr, reg_addr, (uint8_t)reg_data);
}
...
static int gc0328_set_contrast(sensor_t *sensor, int level)
{
	...
	cambus_writeb3(sensor->slv_addr, 0xfe, 0x00);
	cambus_writeb3(sensor->slv_addr, 0xd4, contrast_regs[level][0]);
	cambus_writeb3(sensor->slv_addr, 0xd3, contrast_regs[level][1]);
    return ret;
}
...
static int gc0328_set_saturation(sensor_t *sensor, int level)
{
	...
	cambus_writeb3(sensor->slv_addr, 0xfe, 0x00);
	cambus_writeb3(sensor->slv_addr, 0xd0, saturation_regs[level][0]);
	cambus_writeb3(sensor->slv_addr, 0xd1, saturation_regs[level][1]);
	cambus_writeb3(sensor->slv_addr, 0xd2, saturation_regs[level][2]);
    return ret;
}
...
static int gc0328_set_auto_exposure(sensor_t *sensor, int enable, int exposure_us)
{
	...
	uint8_t temp;
	cambus_writeb3(sensor->slv_addr, 0xfe, 0x00);
	cambus_readb3(sensor->slv_addr, 0x4f, &temp);
	if(enable != 0)
	{
		cambus_writeb3(sensor->slv_addr,0x4f, temp|0x01); // enable
		if(exposure_us != -1)
		{
			cambus_writeb3(sensor->slv_addr, 0xfe, 0x01);
			cambus_writeb3(sensor->slv_addr,0x2f, (uint8_t)(((uint16_t)exposure_us)>>8));
			cambus_writeb3(sensor->slv_addr,0x30, (uint8_t)(((uint16_t)exposure_us)));
		}
	}
	else
	{
		cambus_writeb3(sensor->slv_addr,0x4f, temp&0xfe); // disable
		if(exposure_us != -1)
		{
			cambus_writeb3(sensor->slv_addr, 0xfe, 0x01);
			cambus_writeb3(sensor->slv_addr,0x2f, (uint8_t)(((uint16_t)exposure_us)>>8));
...
static int gc0328_set_auto_whitebal(sensor_t *sensor, int enable, float r_gain_db, float g_gain_db, float b_gain_db)
{
	...
	uint8_t temp;
	cambus_writeb3(sensor->slv_addr, 0xfe, 0x00);
	cambus_readb3(sensor->slv_addr, 0x42, &temp);
	if(enable != 0)
	{
		cambus_writeb3(sensor->slv_addr,0x42, temp|0x02); // enable
		if(!isnanf(r_gain_db))
			cambus_writeb3(sensor->slv_addr,0x80, (uint8_t)r_gain_db); //limit
		if(!isnanf(g_gain_db))
			cambus_writeb3(sensor->slv_addr,0x81, (uint8_t)g_gain_db);
		if(!isnanf(b_gain_db))
			cambus_writeb3(sensor->slv_addr,0x82, (uint8_t)b_gain_db);
	}
	else
	{
		cambus_writeb3(sensor->slv_addr,0x42, temp&0xfd); // disable
		if(!isnanf(r_gain_db))
			cambus_writeb3(sensor->slv_addr,0x77, (uint8_t)r_gain_db);
		if(!isnanf(g_gain_db))
			cambus_writeb3(sensor->slv_addr,0x78, (uint8_t)g_gain_db);
		if(!isnanf(b_gain_db))
			cambus_writeb3(sensor->slv_addr,0x79, (uint8_t)b_gain_db);
	}
    return ret;
}
...
{
    uint8_t data;
    cambus_readb3(GC0328_ADDR, 0x17, &data);
	data = data & 0xFC;
    if(enable){
    ...
    return cambus_writeb3(GC0328_ADDR, 0x17, data);
}
...
int gc0328_reset(sensor_t* sensor)
{
	...
	// open_gc0328_1();
    cambus_writeb3(GC0328_ADDR, 0xfe, 0x01);
    for (index = 0; gc0328_config[index][0]; index++)
    {
        if(gc0328_config[index][0] == 0xff){
            systick_sleep(gc0328_config[index][1]);
            continue;
        }
        // mp_printf(&mp_plat_print, "0x12,0x%02x,0x%02x,\r\n", sensor_default_regs[index][0], sensor_default_regs[index][1]);//debug
        cambus_writeb3(GC0328_ADDR, gc0328_config[index][0], gc0328_config[index][1]);
        // systick_sleep(1);
    }
	systick_sleep(1);
	sccb_i2c_switch(0);
...
int gc0328_set_windowing(framesize_t framesize, int x, int y, int w, int h)
{
	if(framesize == FRAMESIZE_QVGA)
	{
		...
		cambus_writeb3(GC0328_ADDR, 0x51, (y>>8) & 0xff);
		cambus_writeb3(GC0328_ADDR, 0x52, y & 0xff);
		cambus_writeb3(GC0328_ADDR, 0x53, (x>>8) & 0xff);
		cambus_writeb3(GC0328_ADDR, 0x54, x & 0xff);
		cambus_writeb3(GC0328_ADDR, 0x55, (h>>8) & 0x01);
		cambus_writeb3(GC0328_ADDR, 0x56, h & 0xff);
		cambus_writeb3(GC0328_ADDR, 0x57, (w>>8) & 0x03);
		cambus_writeb3(GC0328_ADDR, 0x58, w & 0xff);
	}
	return 0;
}
